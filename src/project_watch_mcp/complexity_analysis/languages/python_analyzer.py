...
class PythonComplexityAnalyzer(BaseComplexityAnalyzer):
    ...
    def _calculate_cognitive_recursive(self, node: Any, nesting: int) -> int:
        ...
        if isinstance(node, (ast.If, ast.While, ast.For)):
            ...
            if hasattr(node, 'orelse') and node.orelse:
                # else clause gets +1 but doesn't increase nesting further
                complexity += 1
                for child in node.orelse:
                    # elif is another If node, else is other statements
                    if isinstance(child, ast.If):
                        # elif - continue at same nesting level
                        complexity += self._calculate_cognitive_recursive(child, nesting)
                    else:
                        # else body
                        complexity += self._calculate_cognitive_recursive(child, local_nesting)
    ...
    def _estimate_cognitive_complexity(self, code: str, radon_item: Any) -> int:
        ...
        # This is a rough estimation since radon doesn't provide cognitive complexity
        # Use cyclomatic as base and adjust for nesting
        base = radon_item.complexity
        
        # Add penalty for deep nesting (simplified)
        if hasattr(radon_item, 'real_complexity'):
            return int(radon_item.real_complexity * 1.2)
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    class_methods = [
                        ...
                        if f.line_start >= node.lineno and f.line_end <= node.end_lineno
                    ]
...
class ComplexityVisitor(ast.NodeVisitor):
    ...
    def _calculate_cognitive_node(self, node, nesting):
        """Calculate cognitive complexity for a single node."""
        complexity = 0
        
        if isinstance(node, ast.If):
            # If statement: +1 + nesting
            complexity += 1 + nesting
            # Process if body
            for child in node.body:
            ...
            if node.orelse:
                if len(node.orelse) == 1 and isinstance(node.orelse[0], ast.If):
                    # elif case - adds +1 but stays at same nesting
                    complexity += 1
                    complexity += self._calculate_cognitive_node(node.orelse[0], nesting)
                else:
                    # else case - adds +1
                    complexity += 1
                    for child in node.orelse:
