...
class TestEmbeddingConfig:
    ...
    def test_openai_provider(self):
        ...
        config = EmbeddingConfig(
            provider="openai", api_key="test-key", model="text-embedding-ada-002"
        )
    ...
    def test_voyage_provider(self):
        """Test Voyage embeddings provider configuration."""
        config = EmbeddingConfig(provider="voyage", api_key="test-key", model="voyage-code-3")
    ...
    def test_from_env_voyage(self):
        """Test loading Voyage embeddings config from environment."""
        with patch.dict(
            os.environ,
            {
    ...
    def test_from_env_invalid_provider(self):
        """Test loading with invalid provider falls back to disabled."""
    ...
    def test_from_env_with_defaults(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            config = EmbeddingConfig.from_env()
            assert config.provider == "voyage"  # Default is voyage now
            assert config.api_key is None
            assert config.model == "voyage-code-3"
