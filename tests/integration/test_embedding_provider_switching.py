...
class TestNeo4jRAGProviderIntegration:
    ...
    def test_neo4j_rag_with_mock_provider(self, mock_neo4j_driver):
        ...
        mock_provider = TestEmbeddingsProvider(dimension=768)
        rag = Neo4jRAG(
    ...
    def test_neo4j_rag_default_provider_fallback(self, mock_neo4j_driver):
        """Test Neo4jRAG falls back to mock when no provider specified and no API keys."""
        with patch.dict(os.environ, {}, clear=True):
            rag = Neo4jRAG(
                neo4j_driver=mock_neo4j_driver,
                project_name="test-project",
                embeddings=None,  # No provider specified
            )
            assert isinstance(rag.embeddings, TestEmbeddingsProvider)
...
class TestProviderSwitchingScenarios:
    ...
    async def test_benchmark_different_providers(self, mock_neo4j_driver):
        ...
        providers.append(("mock", TestEmbeddingsProvider()))
